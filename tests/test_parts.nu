...
def "extracts kind, scope and description" [] {
    let p = ('feat(ui): add picker' | parts)
...
def "lowercases the kind" [] {
    assert equal ('FIX: typo' | parts | get kind) "fix"
    assert equal ('Feat: stuff' | parts | get kind) "feat"
}
...
def "conventional flag is true for valid subject" [] {
    assert equal ('feat: x' | parts | get conventional) true
}

@test
def "raw subject is preserved verbatim" [] {
    let p = ('FIX(API)!: typo' | parts)
...
def "sets breaking when ! is in the prefix" [] {
    assert equal ('feat!: x' | parts | get breaking) true
    assert equal ('feat(api)!: x' | parts | get breaking) true
}

@test
def "sets breaking from BREAKING CHANGE footer" [] {
    let p = ("feat: x\n\nBREAKING CHANGE: drop /v1" | parts)
    assert equal $p.breaking true
}

@test
def "sets breaking from BREAKING-CHANGE hyphen variant" [] {
    let p = ("feat: x\n\nBREAKING-CHANGE: yep" | parts)
    assert equal $p.breaking true
}
...
def "breaking is false without ! or BREAKING footer" [] {
    assert equal ('feat: x' | parts | get breaking) false
    assert equal ("feat: x\n\nbody\n\nRefs: 1" | parts | get breaking) false
}
