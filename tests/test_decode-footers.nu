...
def "single footer with colon-space separator" [] {
    assert equal (decode-footers ['Refs: 42']) [{token: "Refs", value: "42"}]
}
...
def "captures BREAKING CHANGE as a single token with embedded space" [] {
    assert equal (decode-footers ['BREAKING CHANGE: drop /v1']) [{token: "BREAKING CHANGE", value: "drop /v1"}]
}

@test
def "captures BREAKING-CHANGE hyphen variant" [] {
    assert equal (decode-footers ['BREAKING-CHANGE: yep']) [{token: "BREAKING-CHANGE", value: "yep"}]
}
...
def "footer with empty value is captured" [] {
    ...
    assert equal (decode-footers ['Refs: ']) [{token: "Refs", value: ""}]
}
